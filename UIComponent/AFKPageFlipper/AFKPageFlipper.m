...
@synthesize flipLeftRight = _flipLeftRight;
...
- (void) initFlip {
    ...
    if (self.flipLeftRight)
        ...
        leftLayer.contentsGravity = kCAGravityBottom;
    ...
    if (self.flipLeftRight)
        ...
        rightLayer.contentsGravity = kCAGravityTop;
	
	[backgroundAnimationLayer addSublayer:rightLayer];
	
	if (flipDirection == AFKPageFlipperDirectionRight || flipDirection == AFKPageFlipperDirectionDown) {
	...
	if (flipDirection == AFKPageFlipperDirectionRight || flipDirection == AFKPageFlipperDirectionDown) {
        ...
        if (self.flipLeftRight) {
            ...
            transform = CATransform3DMakeRotation(0.0, 1.0, 0.0, 0.0);
            transform.m34 = -1.0f / 2500.0f;
        }
        ...
        if (self.flipLeftRight) {
            ...
            backLayer.contentsGravity = kCAGravityBottom;
            frontLayer.contentsGravity = kCAGravityTop;
            transform = CATransform3DMakeRotation(-M_PI / 1.1, 1.0, 0.0, 0.0);
            transform.m34 = -1.0f / 2500.0f;
        }
...
- (void) cleanupFlip {
    ...
    // do cleanup on when currentView and nextView is not nil
//    NSLog(@"current: %@, next: %@", self.currentView, self.nextView);
    ...
    if ([self.delegate respondsToSelector:@selector(pageFlipper:didFlipped:)])
        [self.delegate pageFlipper:self didFlipped:currentPage];
}
...
- (void) refreshPage {
    [self.currentView removeFromSuperview];
	self.currentView = [self.dataSource viewForPage:currentPage inFlipper:self];
...
- (void) tapped:(UITapGestureRecognizer *) recognizer {
	...
	if (recognizer.state == UIGestureRecognizerStateRecognized) {
        ...
        if (self.flipLeftRight) {
            minOffset = self.bounds.size.width/6;
            maxOffset = self.bounds.size.width*5/6;
            tapCenter = [recognizer locationInView:self].x < maxOffset && [recognizer locationInView:self].x > minOffset;
        } else {
            minOffset = self.bounds.size.height/6;
            maxOffset = self.bounds.size.height*5/6;            
            tapCenter = [recognizer locationInView:self].y < maxOffset && [recognizer locationInView:self].y > minOffset;
        }
        if (tapCenter) {
            if ([self.delegate respondsToSelector:@selector(pageFlipperSingleTapped)])
                [self.delegate pageFlipperSingleTapped];
            return;
        }
...
- (void) panned:(UIPanGestureRecognizer *) recognizer {
    ...
    if (!self.flipLeftRight) {
        translation = [recognizer translationInView:self].y;
        progress = translation / self.bounds.size.height;
    }
	
	if (flipDirection == AFKPageFlipperDirectionLeft || flipDirection == AFKPageFlipperDirectionUp) {
	...
	switch (recognizer.state) {
		...
		case UIGestureRecognizerStateRecognized:
			...
			if (fabs((translation + [recognizer velocityInView:self].x / 4) / self.bounds.size.width) > 0.5
                || fabs((translation + [recognizer velocityInView:self].y / 4) / self.bounds.size.height) > 0.5) {
				setNextViewOnCompletion = YES;
				[self setFlipProgress:1.0 setDelegate:YES animate:YES];
			} else {
				[self setFlipProgress:0.0 setDelegate:YES animate:YES];
